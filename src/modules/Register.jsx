...
export default function Register() {
  ...
  const formFinish = (values) => {
    if (!values.name || !values.email || !values.password) {
      message.warning('Preencha todos os campos.', 4);
    }
    ...
    if (values.email && values.password) {
      ...
      .then((response) => {
        message.destroy();
        message.success('Usuário criado com sucesso! Seja bem vindo(a)!', 4);
      ...
      .catch((error) => {
        message.destroy();
        message.error(error.message || 'Erro ao tentar criar usuário. Tente novamente.', 4);
      })
      .finally(() => {
        form.resetFields();
      });
  ...
  return (
    <Base>
      ...
      <Flex vertical="vertical" justify="center" style={{width: '100%'}}>
        <Title style={{color: '#FFFFFF'}}>Registre-se</Title>
        <Paragraph style={{color: '#FFFFFFBB'}}>
          Crie sua conta e tenha acesso a nossa plataforma com todos os conteúdos exclusivos.
        </Paragraph>

        <Form form={form} onFinish={formFinish} style={{width: '100%'}} layout="vertical">
        <Form.Item name="name" label={<Text style={{color: '#FFFFFF50'}}>Nome</Text>}>
